...
from backbone.margins import Margins
...
class SherlockClassifier(nn.Module):
    def __init__(self,
        ...
        if fc_type is None:
            ...
            self.fc_layer = Margins(embedding_dim, num_classes)
    ...
    def forward(self, X: Dict[str, torch.Tensor], y, fc_type='FC', scale=32, t=0.0, margin=0.0, decisive_margin=0.0):
        ...
        if fc_type is None:
            ...
            out = self.fc_layer(self.l2_norm(out), y, fc_type, scale, t, margin, decisive_margin)
        return out

    def predict(self,
                X: Dict[str, torch.Tensor], y, fc_type):
        return F.softmax(self.forward(X, y, fc_type))
...
def build_sherlock(fc_type, arg_feature_groups, num_classes, topic_dim=None, dropout_ratio=0.5):
    ...
    classifier = SherlockClassifier(fc_type, feature_enc_dict, num_classes=num_classes, dropout_ratio=dropout_ratio)
    return classifier
