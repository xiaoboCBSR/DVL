...
def get_predictions(path, config_name, fc_type):
    ...
    y_pred = np.load(join(path, 'y_pred_{}_{}.npy'.format(config_name, fc_type)))
    y_true = np.load(join(path, 'y_true.npy'))

    return y_pred, y_true


def report_gen(y_pred, y_true, report_name=None, out_loc=join('results', 'report')):
    ...
    df_report['mis_from_top5'] = df_report.apply(lambda x: first_five(dic[int(x['type'])]['mis_from']),
                                                 axis=1)  # precision
    df_report['mis_to_top5'] = df_report.apply(lambda x: first_five(dic[int(x['type'])]['mis_to']), axis=1)  # recall
...
def CRF_tuple_gen(path, LDA_tag):
    ...
    tuple_list.append(
        (LDA_tag, 'sherlock', res_sherlock['macro avg']['f1-score'], res_sherlock['weighted avg']['f1-score']))
...
#         os.makedirs(loc)
#
#     y_predict, y_true = get_predictions(path)
#     # y_predict_LDA, _ = get_predictions(path_LDA)
#
#     result_sherlock = report_gen(y_predict, y_true)
#
#
...
def data_gen_all(path, config_name, fc_type, loc):
    if not os.path.exists(loc):
        os.makedirs(loc)

    y_predict, y_true = get_predictions(path, config_name, fc_type)
    result = report_gen(y_predict, y_true)
    print(result[1])
    result[1].to_csv(join(loc, 'sort_result_{}_{}.csv'.format(config_name, fc_type)))
