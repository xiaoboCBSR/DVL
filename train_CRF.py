...
p.add('--learning_rate', type=float, default=0.01) #default 0.01
p.add('--decay', type=float, default=1e-4)  # default 1e-4
p.add('--dropout_rate', type=float, default=0.35) #default 0.35
...
print(pre_trained_sherlock_loc)
...
if init_matrix_path is None:
    ...
    init_matrix_loc = join('results', 'co_occur_matrix')
...
def eval_batch(table_batch, label_batch, mask_batch, fc_type):
    ...
    if fc_type is None:
        ...
        emissions = classifier(table_batch, label_batch.view(-1), fc_type='FC').view(batch_size_new, MAX_COL_COUNT, -1)
...
for corpus in corpus_list:
    ...
    if args.mode != 'eval':
        ...
        # train = copy.copy(whole_corpus).set_filter(train_ids).to_col(args.mode, num_classes, noise_type, noise_rate)
...
if args.mode == 'train':
    ...
    with torch.no_grad():
        ...
        for table_batch, label_batch, mask_batch in tqdm(validation, desc='Single col Accuracy'):
            for f_g in table_batch:
                ...
                #     table_batch[f_g] = table_batch[f_g].view(batch_size * MAX_COL_COUNT, -1)
                # except:
                #     print(batch_size * MAX_COL_COUNT)
                #     a = table_batch

            if fc_type is None:
                pred_scores = classifier.predict(table_batch, label_batch.view(-1), fc_type)
            else:
                pred_scores = classifier.predict(table_batch, label_batch.view(-1), fc_type='FC')
    ...
    for epoch_idx in range(epochs):
        ...
        # np.save(join(logging_path, "outputs", 'y_pred_epoch_{}.npy'.format(epoch_idx)),
    ...
    torch.save({'col_classifier': classifier.state_dict(), 'CRF_model': model.state_dict()},
               join(logging_path, "{}_{}".format(logging_name, pre_trained_sherlock_path)))
...
# evaluation mode
    ...
    for model_path in args.model_list:
        ...
        with torch.no_grad():
            ...
            df.to_csv(join(logging_path, "result_{}_{}.csv".format(config_name, fc_type)), index=True)
